...
var basePath = string.IsNullOrWhiteSpace(fileOptions.BasePath)
    ? "uploads"
    : fileOptions.BasePath;

var requestPath = string.IsNullOrWhiteSpace(fileOptions.RequestPath)
    ? "/uploads"
    : fileOptions.RequestPath;
...
var absoluteBasePath = Path.IsPathRooted(basePath)
    ? basePath
    : Path.GetFullPath(Path.Combine(app.Environment.ContentRootPath, basePath));
...
// Swagger включаем и в Azure тоже
app.UseSwagger();
app.UseSwaggerUI();
...
// Простые эндпоинты для быстрой проверки, что приложение живо
