...
public sealed class SetBankAccountCommandHandler
    : IRequestHandler<SetBankAccountCommand, Result>
{
    ...
    public SetBankAccountCommandHandler(
    ...
    public async Task<Result> Handle(SetBankAccountCommand request, CancellationToken ct)
    {
        ...
        var bankResult = BankAccount.Create(
        ...
        if (bankResult.IsFailure)
            return Result.Failure(bankResult.Error);

        var result = profile.SetBankAccount(bankResult.Value!);
        if (result.IsFailure)
            return result;
