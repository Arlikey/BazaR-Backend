...
public sealed class UpdateMyProfileCommandHandler : IRequestHandler<UpdateMyProfileCommand, Result>
{
    ...
    public UpdateMyProfileCommandHandler(IUserRepository users, ICurrentUser current, IUnitOfWork uow)
    {
        _users = users;
        _current = current;
        _uow = uow;
    ...
    public async Task<Result> Handle(UpdateMyProfileCommand request, CancellationToken ct)
    {
        ...
        var res = user.UpdateProfile(request.FirstName, request.LastName, request.Phone);
