...
public sealed class MarkShippingReadyToShipCommandHandler
    : IRequestHandler<MarkShippingReadyToShipCommand, Result>
{
    ...
    public MarkShippingReadyToShipCommandHandler(
    ...
    public async Task<Result> Handle(MarkShippingReadyToShipCommand request, CancellationToken ct)
    {
        ...
        var result = shipping.MarkReadyToShip();
        if (result.IsFailure)
            return result;
