...
public sealed class EnableShippingMethodCommandHandler
    : IRequestHandler<EnableShippingMethodCommand, Result>
{
    ...
    public EnableShippingMethodCommandHandler(
    ...
    public async Task<Result> Handle(EnableShippingMethodCommand request, CancellationToken ct)
    {
        ...
        var result = profile.EnableMethod(request.MethodType);
        if (result.IsFailure)
            return result;
